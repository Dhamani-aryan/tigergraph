...
- **Case memory within the run** (spec §6 step 8) had a real bug in the first draft — new case vertices (named `FraudCase` after Task 4's live rename, see below) were written but never embedded, so `retrieve_knowledge` could never find them. Fixed by embedding on write in `_write_case_to_graph` (Task 12) and having `retrieve_knowledge` (Task 10) search `FraudCase` alongside `ClosedCase`.
